class Solution(object):
    def maxNumber(self, nums1, nums2, k):
        ...
        :rtype: List[int]
        """
        start_indexs = set([(0, 0)])
        values = []
        while k > 0:
            tmp_value = -1
            tmp_indexs = set()
            for indexs in start_indexs:
                v, i = self.getNextValue(nums1, nums2, k, *indexs)
                if v < tmp_value:
                    continue
                elif v == tmp_value:
                    tmp_indexs.update(i)
            ...
            start_indexs = tmp_indexs
            k -= 1
        return values

    def getNextValue(self, nums1, nums2, k, index1, index2):
        """ get next value

        :param nums1: List[int]
        :param nums2:  List[int]
        :param k: int
        :param index1: start index of nums1 (value index include index1)
        :param index2: start index of nums2 (value index include index2)
        :return: new_value, [(new_index1, new_index2), ...]
        """
        new_index1, value1 = self.getNextValueForNums1(nums1, nums2, k, index1, index2)
        new_index2, value2 = self.getNextValueForNums1(nums2, nums1, k, index2, index1)
        if value1 < value2:
            return value2, [(index1, new_index2)]
        elif value1 > value2:
            return value1, [(new_index1, index2)]
        else:
            return value1, [(index1, new_index2), (new_index1, index2)]

    def getNextValueForNums1(self, nums1, nums2, k, index1, index2):
        """ get max index of nums1 for next value
        ...
        max_index = len(nums1) - k + len(nums2) - index2
        max_index = min(len(nums1) - 1, max_index)

        for i in range(index1, max_index + 1):
            if nums1[index1] < nums1[i]:
                index1 = i
        return index1 + 1, nums1[index1]


# print Solution().maxNumber([3, 4, 6, 5], [9, 1, 2, 5, 8, 3], 5)
# for n in range(1, 11):
#     print Solution().maxNumber([3, 4, 6, 5], [9, 1, 2, 5, 8, 3], n)
